...
echo "Preparing openclaw Agent for NuGuard Testing..."
echo "Log: $LOG_FILE"
...
uv run nuguard sbom generate \
  ...
  --format json \
  -o "$SCRIPT_DIR/openclaw.sbom.json"
...
uv run nuguard policy check \
  --config "$SCRIPT_DIR/nuguard.yaml" \
  --format markdown \
  -o "$SCRIPT_DIR/reports/openclaw-policy-check.md" || true
...
uv run nuguard analyze \
  ...
  --verbose \
  --format markdown \
  -o "$SCRIPT_DIR/reports/openclaw-analysis.md" || true
...
uv run nuguard behavior \
  ...
  --mode static+dynamic \
  --format markdown \
  -o "$SCRIPT_DIR/reports/openclaw-behavior.md" || true

echo "---"
echo "Done: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "Log saved to: $LOG_FILE"
echo "Report:       $SCRIPT_DIR/reports/openclaw-behavior.md"
...
uv run nuguard redteam \
  --config "$SCRIPT_DIR/nuguard.yaml" \
  --format markdown \
  --output "$SCRIPT_DIR/reports/openclaw-redteam.md" || true
