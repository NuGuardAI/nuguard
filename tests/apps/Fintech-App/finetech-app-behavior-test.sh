...
uv run nuguard sbom generate \
  ...
  --format json \
  -o "$SCRIPT_DIR/fintech-app.sbom.json"
...
uv run nuguard policy check \
  ...
  --format markdown \
  -o "$SCRIPT_DIR/reports/fintech-app-policy-check.md" || true
...
uv run nuguard behavior \
  ...
  --mode static+dynamic \
  --format markdown \
  -o "$SCRIPT_DIR/reports/fintech-app-behavior.md" || true

echo "---"
echo "Done: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "Log saved to: $LOG_FILE"
echo "Report:       $SCRIPT_DIR/reports/fintech-app-behavior.md"
