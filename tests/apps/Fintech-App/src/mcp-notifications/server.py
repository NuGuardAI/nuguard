...
logger = _setup_logger("mcp_notifications")
...
def _summarize_arguments(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if "channel" in arguments:
        summary["channel"] = str(arguments["channel"])
    if "message" in arguments:
        message = str(arguments["message"])
        summary["message_len"] = len(message)
        summary["message_preview"] = message[:48]
    if "phone_number" in arguments:
        summary["phone_present"] = bool(arguments["phone_number"])
    return summary
