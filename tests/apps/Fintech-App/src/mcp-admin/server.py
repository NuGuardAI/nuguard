...
logger = _setup_logger("mcp_admin")
...
def _summarize_arguments(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if "user_id" in arguments:
        summary["user_id"] = str(arguments["user_id"])
    if "role" in arguments:
        summary["role"] = str(arguments["role"])
    if "granted_by" in arguments:
        summary["granted_by"] = str(arguments["granted_by"])
    if "reason" in arguments:
        reason = str(arguments["reason"])
        summary["reason_len"] = len(reason)
        summary["reason_preview"] = reason[:48]
    if "new_password" in arguments:
        summary["new_password_present"] = bool(arguments["new_password"])
    return summary
