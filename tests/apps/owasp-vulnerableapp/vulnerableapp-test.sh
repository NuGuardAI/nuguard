...
echo "Starting OWASP VulnerableApp via deploy-local.sh..."
"$SCRIPT_DIR/deploy-local.sh" &
DEPLOY_PID=$!

cleanup() {
  echo "Stopping OWASP VulnerableApp..."
...
for _ in $(seq 1 60); do
  if curl -s -o /dev/null "$TARGET_URL"; then
    echo "OWASP VulnerableApp is up."
    break
  fi
  sleep 2
done
...
uv run nuguard sbom generate \
  ...
  --format json \
  -o "$SCRIPT_DIR/vulnerableapp.sbom.json"

echo "SBOM generated successfully."

echo "Initializing Cognitive Policy controls..."

uv run nuguard policy draft --sbom "$SCRIPT_DIR/vulnerableapp.sbom.json" --output "$SCRIPT_DIR/cognitive_policy.md" --force
...
uv run nuguard analyze \
  ...
  --verbose \
  --format markdown \
  -o "$SCRIPT_DIR/reports/vulnerableapp-analysis.md" || true
...
uv run nuguard behavior \
  ...
  --mode static+dynamic \
  --format markdown \
  -o "$SCRIPT_DIR/reports/vulnerableapp-behavior.md" || true

echo "---"
echo "Done: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "Log saved to: $LOG_FILE"
echo "Report:       $SCRIPT_DIR/reports/vulnerableapp-behavior.md"
  
echo "Running pentest for OWASP VulnerableApp ..."
...
uv run nuguard pentest  \
  ...
  --format markdown \
  --output "$SCRIPT_DIR/reports/vulnerableapp-pentest.md" || true

echo "---"
echo "Running redteam tests for OWASP VulnerableApp ..."

# redteam tests exit 2 when findings are present — expected in testing; treat as non-fatal.
uv run nuguard redteam \
  --config "$SCRIPT_DIR/nuguard.yaml" \
  --format markdown \
  --output "$SCRIPT_DIR/reports/vulnerableapp-redteam.md" || true
