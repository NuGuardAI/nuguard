...
**FinTech GOAT** (*Goat of All Threats*) is a fully synthetic, AI-powered banking application modelled after real-world FinTech architectures. It contains **46 deliberately planted security vulnerabilities** spanning:
...
|                                      +-----------------------------------+   |
|                                                                               |
|  Supporting: Azure OpenAI . App Insights                                     |
+------------------------------------------------------------------------------+
```
...
For day-to-day frontend and backend changes, run the UI and orchestrator in hot-reload mode:

```bash
bash dev.sh
```

This starts Vite on `http://localhost:5173` with `/api` and `/ws` proxied to the orchestrator on `http://localhost:8001`, and runs the backend with uvicorn reload enabled.
...
| Username | Password | Checking | Savings | Investments |
|---|---|---|---|---|
| `alice` | `demo123` | $50,000 | $18,420 | $37,834 |
| `bob` | `demo123` | $12,300 | $5,600 | $8,900 |
| `carol` | `demo123` | $250,000 | $0 | $0 |
| `david` | `demo123` | $3,400 | $1,200 | $500 |
| `eve` | `demo123` | $88,000 | $42,000 | $120,000 |
| `admin` | `Admin@2024!` | — | — | — |

Authentication uses `POST /api/auth/login` with `{"email": "<email>", "password": "<password>"}`.  
The UI login form now calls the backend directly. Pass any username as `user_id` in the chat body to exploit VULN-AI-02 (IDOR) regardless of the JWT.
...
```bash
# Prerequisites: Azure Developer CLI (azd), Docker, and access to the
# NuGuard-AI Platform subscription: fb4c60ce-198d-408f-b426-e1fe06fd704a

# 1. Authenticate
azd auth login

# 2. Create/select an azd environment and pin it to East US
azd env new Refined-FinTech-App --subscription fb4c60ce-198d-408f-b426-e1fe06fd704a --location eastus
azd env set AZURE_SUBSCRIPTION_ID fb4c60ce-198d-408f-b426-e1fe06fd704a
azd env set AZURE_LOCATION eastus
azd env set AZURE_RESOURCE_GROUP rg-nuguard-ai-platform-Refined-FinTech-App
azd env set JWT_SECRET "$(openssl rand -hex 32)"

# 3. Deploy all infrastructure and container images
azd up

# Outputs:
#   SERVICE_FRONTEND_UI_URI           - public URL for the banking UI
#   SERVICE_AGENT_ORCHESTRATOR_URI    - orchestrator API base URL
#   SERVICE_MCP_BANKING_SERVER_URI    - internal banking MCP URL
```

The Bicep templates in `infra/` provision:
- Resource group `rg-nuguard-ai-platform-${AZURE_ENV_NAME}` in East US
- Azure Container Registry for `azd` image publishing
- Azure Container Apps environment with all 23 services
- Azure OpenAI (GPT-4o)
- Log Analytics workspace + Application Insights

The Azure templates intentionally do not create `Microsoft.AlertsManagement`
resources, action groups, or alert rules.

---

## Running NuGuard Against the GOAT

NuGuard v0.2.0 was used to run a complete 40-test suite against this application. See [nuguard-test-results/TESTING_REPORT.md](nuguard-test-results/TESTING_REPORT.md) for the full results.

### Quick NuGuard Setup

```bash
# Install NuGuard (requires uv)
git clone https://github.com/nuguard/nuguard.git
cd nuguard
uv sync

# Set Azure OpenAI environment variables
export AZURE_API_KEY="your-key"
export AZURE_API_BASE="https://your-aoai.openai.azure.com/"
export AZURE_API_VERSION="2024-05-01-preview"
export NUGUARD_REDTEAM_LLM_MODEL="azure/gpt-4o"
export NUGUARD_REDTEAM_LLM_API_KEY="your-key"
```

### NuGuard Command Reference (Tested Against This App)

```bash
# 1. Generate the AI-SBOM (JSON, CycloneDX, Markdown)
uv run nuguard sbom generate --source ./src --format json --output app.sbom.json
uv run nuguard sbom generate --source ./src --format cyclonedx --output app.cyclonedx.json
uv run nuguard sbom generate --source ./src --format markdown --output app.sbom.md

# 2. Validate SBOM
uv run nuguard sbom validate --sbom app.sbom.json

# 3. Run SBOM plugins
uv run nuguard sbom plugin list --sbom app.sbom.json
uv run nuguard sbom plugin run markdown_export --sbom app.sbom.json
uv run nuguard sbom plugin run dependency_analyze --sbom app.sbom.json
uv run nuguard sbom plugin run license_check --sbom app.sbom.json
uv run nuguard sbom plugin run vulnerability --sbom app.sbom.json

# 4. Static analysis
uv run nuguard analyze --sbom app.sbom.json --format markdown --min-severity low
uv run nuguard analyze --sbom app.sbom.json --format json --min-severity critical
uv run nuguard analyze --sbom app.sbom.json --format sarif
uv run nuguard analyze --sbom app.sbom.json --llm --format markdown   # LLM remediation

# 5. Policy checks
uv run nuguard policy validate --policy cognitive_policy.md
uv run nuguard policy check --policy cognitive_policy.md --sbom app.sbom.json
uv run nuguard policy check --framework owasp-llm-top10
uv run nuguard policy check --framework nist-ai-rmf
uv run nuguard policy check --framework eu-ai-act
uv run nuguard policy check --framework owasp-llm-top10 --llm --format json

# 6. Full scan pipeline (SBOM + analyze + policy in one command)
uv run nuguard scan --steps sbom,analyze --format markdown
uv run nuguard scan --steps sbom,analyze,policy --policy cognitive_policy.md
uv run nuguard scan --steps sbom,analyze --llm

# 7. AI Red-teaming (most powerful feature)
# Note: Requires NUGUARD_REDTEAM_LLM_MODEL and associated API key env vars
...
# Red-team with canary data (detects data exfiltration)
uv run nuguard redteam --sbom app.sbom.json --target <URL> \
    --profile ci --canary canary.json --format json

# Red-team with policy awareness
uv run nuguard redteam --sbom app.sbom.json --target <URL> \
    --profile ci --policy cognitive_policy.md --format json

# Filter by scenario type (use substring of enum values)
# Valid: injection, data_exfiltration, tool_abuse, mcp, privilege, context, structural
uv run nuguard redteam --sbom app.sbom.json --target <URL> \
    --scenarios data_exfiltration --format json

# Guided multi-turn attacks (adversarial conversation)
uv run nuguard redteam --sbom app.sbom.json --target <URL> \
    --profile ci --guided --guided-max-turns 15 --format json

# Set fail threshold for CI/CD integration
uv run nuguard redteam --sbom app.sbom.json --target <URL> \
    --profile ci --fail-on medium --format json
```
