...
async def tools_call_endpoint(request: Request) -> JSONResponse:
    ...
    try:
        ...
        logger.warning("tools/call invalid params tool=%s: %s", tool_name, exc)
        return JSONResponse({"jsonrpc": "2.0", "id": call_id, "error": {"code": -32602, "message": f"Invalid params: {exc}"}}, status_code=400)
    except Exception as exc:
        logger.error("tools/call error tool=%s", tool_name, exc_info=True)
        return JSONResponse({"jsonrpc": "2.0", "id": call_id, "error": {"code": -32603, "message": "Internal server error"}}, status_code=500)
