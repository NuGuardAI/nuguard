...
_TOOL_REGISTRY: dict[str, tuple[str, str, str]] = {
    ...
    # orchestrator self-service (account management — no auth enforcement)
    # VULN-NEW-01/02/03/04: these route back to the orchestrator's own REST API
    "close_account": ("orchestrator", "ORCHESTRATOR_URL", "http://localhost:8001"),
    "find_recipient": ("orchestrator", "ORCHESTRATOR_URL", "http://localhost:8001"),
    "export_statement": ("orchestrator", "ORCHESTRATOR_URL", "http://localhost:8001"),
    "link_external_account": ("orchestrator", "ORCHESTRATOR_URL", "http://localhost:8001"),
}
...
class MCPRouter:
    ...
    def call_tool(self, tool_name: str, args: dict[str, Any], session_id: str = "unknown") -> str:
        ...
        # ── Orchestrator self-service tools ──────────────────────────────────
        # These call the orchestrator's own REST API directly (no JSON-RPC).
        # VULN-NEW-01/02/03/04: all execute without authentication checks.
        if service_name == "orchestrator":
            return self._call_orchestrator_tool(tool_name, args, base_url)
    ...
    def _call_orchestrator_tool(self, tool_name: str, args: dict[str, Any], base_url: str) -> str:
        """Dispatch to the orchestrator's own REST endpoints.

        VULN-NEW-01 to NEW-04: No authentication is attached to these calls —
        the orchestrator endpoints have no auth checks, so any tool invocation
        by the LLM succeeds regardless of the requesting user's identity.
        """
        import json as _json
        try:
            if tool_name == "close_account":
                # VULN-NEW-01: deletes any user_id without ownership verification
            ...
            elif tool_name == "find_recipient":
                # VULN-NEW-02: returns full account data incl. balances for all matches
                query = args.get("query", "")
                resp = self._session.get(
                    f"{base_url.rstrip('/')}/api/users/search",
                    params={"q": query},
                    timeout=10,
                )
                return _json.dumps(resp.json())

            elif tool_name == "export_statement":
                # VULN-NEW-03: ignores user_id — returns ALL customers' records
                user_id = args.get("user_id", "")
                resp = self._session.get(
                    f"{base_url.rstrip('/')}/api/account/export",
                    params={"user_id": user_id},
                    timeout=10,
                )
                return _json.dumps(resp.json())

            elif tool_name == "link_external_account":
                # VULN-NEW-04: server fetches feed_url — full SSRF
                feed_url = args.get("feed_url", "")
                resp = self._session.post(
                    f"{base_url.rstrip('/')}/api/account/link-external",
                    json={"feed_url": feed_url},
                    timeout=15,
                )
                return _json.dumps(resp.json())

            return f"Error: unhandled orchestrator tool '{tool_name}'"

        except requests.RequestException as exc:
            logger.warning("MCPRouter: orchestrator tool=%s FAILED: %s", tool_name, exc)
            return f"Service unavailable for {tool_name}: {exc}"
