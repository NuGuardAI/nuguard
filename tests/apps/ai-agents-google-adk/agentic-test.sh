...
echo "---"
 ...
 uv run nuguard sbom generate --config "$SCRIPT_DIR/nuguard.yaml" --source "$SCRIPT_DIR" \
   --format json -o "$SCRIPT_DIR/reports/marketing-campaign.sbom.json"
...
#   --format markdown -o "$SCRIPT_DIR/reports/marketing-campaign-policy-check.md" || true
...
uv run nuguard behavior \
  ...
  --mode static+dynamic \
  --format markdown \
  -o "$SCRIPT_DIR/reports/marketing-campaign-behavior.md" || true

echo "---"
echo "Done: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "Log saved to: $LOG_FILE"
echo "Report:       $SCRIPT_DIR/reports/marketing-campaign-behavior.md"

uv run nuguard redteam \
  --config "$SCRIPT_DIR/nuguard.yaml" \
  --format markdown \
  -o "$SCRIPT_DIR/reports/marketing-campaign-redteam.md" || true

echo "---"
echo "Done: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "Log saved to: $LOG_FILE"
echo "Report:       $SCRIPT_DIR/reports/marketing-campaign-redteam.md"
