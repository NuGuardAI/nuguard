...
class TestFlatOrNone:
    def test_none_input_returns_none(self) -> None:
        assert flat_or_none(None) is None

    def test_empty_dict_returns_empty_dict(self) -> None:
        assert flat_or_none({}) == {}

    def test_flat_dict_returned_unchanged(self) -> None:
        flat = {"user_id": "alice", "language": "en"}
        assert flat_or_none(flat) == flat

    def test_nested_dict_returns_none(self) -> None:
        nested = {"message": {"role": "user", "content": MESSAGE}}
        assert flat_or_none(nested) is None

    def test_nested_list_value_returns_none(self) -> None:
        nested = {"messages": [{"role": "user", "content": MESSAGE}]}
        assert flat_or_none(nested) is None
