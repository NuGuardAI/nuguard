...
def test_sql_injection_sqlite_blind_payload_uses_randomblob():
    """SQLite blind step must use RANDOMBLOB (heavy query) — not load_extension or SLEEP."""
    s = build_datastore_sql_injection(
        agent_id="a", agent_name="Agent",
        datastore_name="db", datastore_type="sqlite",
        pii_fields=[],
    ...
    assert "LOAD_EXTENSION" not in payload
    assert "RANDOMBLOB" in payload
...
def test_sql_injection_step3_uses_sbom_table_name():
    """UNION SELECT must reference the actual SBOM table name, not a hardcoded guess."""
    s = build_datastore_sql_injection(
        agent_id="a", agent_name="Agent",
        datastore_name="db", datastore_type="sqlite",
        pii_fields=["account_id", "email"],
        table_names=["fintech_accounts"],
    )
    assert "fintech_accounts" in s.chain.steps[2].payload


def test_sql_injection_step3_falls_back_to_accounts_without_table_names():
    """Without table_names the fallback table used in UNION SELECT must be 'accounts'."""
    s = build_datastore_sql_injection(
        agent_id="a", agent_name="Agent",
        datastore_name="db", datastore_type="sqlite",
        pii_fields=["email"],
    )
    assert "accounts" in s.chain.steps[2].payload


def test_sql_injection_step2_uses_sensible_seed_value_for_email_field():
    """Boolean injection value should look like a real email, not a mismatch."""
    s = build_datastore_sql_injection(
        agent_id="a", agent_name="Agent",
        datastore_name="db", datastore_type="sqlite",
        pii_fields=["email"],
    ...
    payload = s.chain.steps[1].payload
    assert "@" in payload  # seed value looks like an email


def test_sql_injection_step2_uses_account_id_seed_for_non_email_field():
    """For non-email PII fields the seed value should be an account-style ID."""
    s = build_datastore_sql_injection(
        agent_id="a", agent_name="Agent",
        datastore_name="db", datastore_type="sqlite",
...
def test_generator_no_datastore_probe_without_pii_or_classified_fields():
    """Schema probe should NOT be generated for datastores with no PII metadata at all."""
...
def test_generator_produces_schema_probe_with_pii_but_no_classified_fields():
    """Schema probe must be generated when pii_fields exist even without classified_fields."""
    agent = Node(id=uuid.uuid4(), name="Agent", component_type=ComponentType.AGENT,
                 confidence=0.9, metadata=NodeMetadata())
    ds = Node(id=uuid.uuid4(), name="postgres", component_type=ComponentType.DATASTORE,
              confidence=0.9, metadata=NodeMetadata(
...
def test_generator_no_sql_injection_for_mongodb():
    """mongodb contains 'db' as a substring — must NOT trigger SQL injection."""
    agent = Node(id=uuid.uuid4(), name="Agent", component_type=ComponentType.AGENT,
                 confidence=0.9, metadata=NodeMetadata())
    ds = Node(id=uuid.uuid4(), name="mongodb", component_type=ComponentType.DATASTORE,
              confidence=0.9, metadata=NodeMetadata(
...
def test_generator_sql_injection_uses_sbom_table_names():
    """SQL injection UNION SELECT step must reference the actual SBOM table name."""
    sbom = _make_fintech_like_sbom()  # sqlite with classified_fields={"LoginRequest": [...]}
    gen = ScenarioGenerator(sbom)
    scenarios = gen.generate()
    sqli = [s for s in scenarios if s.scenario_type == ScenarioType.DATASTORE_SQL_INJECTION]
    assert sqli, "Expected at least one SQL injection scenario"
    union_step = sqli[0].chain.steps[2]
    assert "LoginRequest" in union_step.payload, (
        f"Expected SBOM table 'LoginRequest' in UNION SELECT payload, got: {union_step.payload!r}"
    )
