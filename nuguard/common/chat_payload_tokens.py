...
def flat_or_none(extras: dict[str, Any] | None) -> dict[str, Any] | None:
    """Return *extras* unchanged when flat/empty, else ``None``.

    Used by callers that read ``chat_payload_extras`` for purposes other than
    building the final request body (endpoint auto-probing, identity-field
    injection) — those assume a flat dict of static values. Feeding them a
    nested slot-mode ``chat_payload_extras`` unchanged would embed literal
    ``"{{message}}"``-style strings into probe requests or misfire identity
    injection, so nested extras are hidden from them entirely rather than
    passed through and misinterpreted.
    """
    if not extras or max_depth(extras) <= 1:
        return extras
    return None
