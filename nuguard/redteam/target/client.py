...
def _log_http_request_body(method: str, path: str, body: Any, *, label: str = "") -> None:
    ...
    caching it, so it can be toggled mid-run (e.g. in a debugger or notebook).
    Labeled distinctly from :func:`_log_http_response_body` so request and
    response lines are unambiguous in the log stream.
    """
    if not env_bool("NUGUARD_LOG_HTTP_BODIES", False):
        return
    suffix = f" {label}" if label else ""
    _log.info(
        "[NUGUARD_LOG_HTTP_BODIES] HTTP %s %s%s: %s",
...
def _log_http_response_body(status: int | str, path: str, body: Any, *, label: str = "") -> None:
    ...
    suffix = f" {label}" if label else ""
    _log.info(
        "[NUGUARD_LOG_HTTP_BODIES] HTTP Response %s %s%s: %s",
...
class TargetAppClient:
    ...
    def _resolve_session_token_context(self, payload: str, session: AttackSession) -> dict[str, Any]:
        ...
        return {
            chat_payload_tokens.MESSAGE: self._build_chat_payload_value(payload, session),
