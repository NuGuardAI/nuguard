...
Produces output aligned with the NuGuardAI SPDX 3.0.1 reference format:
  ...
  * ``"@context"`` is the SPDX 3.0.1 canonical URL string (+ nuguard namespace)
...
# RelationshipType → SPDX 3 relationshipType camelCase string
...
def _convert_dict(
    ...
    agent_id = "SPDXRef-Agent-NuGuardSoftwareAgent"
    tool_id = "SPDXRef-Tool-NuGuardGenerator"
    ...
    graph.append(
        {
            ...
            "spdxId": agent_id,
            "creationInfo": ci_ref,
            "name": "NuGuard AI SBOM Generator Agent",
    ...
    sbom_elem: dict[str, Any] = {
        ...
        "summary": f"AI SBOM for {doc.target} generated by NuGuard.",
...
def validate_spdx3_jsonld(path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        raise ImportError(
            "SHACL validation requires pyshacl and rdflib: pip install nuguard[spdx]"
        ) from ex
    ...
    try:
        ...
        raise FileNotFoundError(
            "SPDX 3 SHACL shapes (model.ttl) not found — install spdx-tools: pip install nuguard[spdx]"
        ) from ex
